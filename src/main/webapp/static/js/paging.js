...
(function(root, factory) {
	...
	Paging.prototype = {
		...
		format: function() {
			var html = '<ul id="page">';
			html += '<li onclick="pageTurning(1,'+this.pagesize+')" class="js-page-first js-page-action ui-pager" >' + this.settings.firstTpl + '</li>';
			var a = (this.current - 1)==0?1:(this.current - 1);
			html += '<li onclick="pageTurning('+a+','+this.pagesize+')" class="js-page-prev js-page-action ui-pager">' + this.settings.prevTpl + '</li>';
			if (this.pagecount > 6) {
				html += '<li onclick="pageTurning(1,'+this.pagesize+')" data-page="1" class="ui-pager">1</li>';
				if (this.current <= 2) {
					html += '<li onclick="pageTurning(2,'+this.pagesize+')" data-page="2" class="ui-pager">2</li>';
					html += '<li onclick="pageTurning(3,'+this.pagesize+')" data-page="3" class="ui-pager">3</li>';
					...
					html += '<li onclick="pageTurning("'+(this.current - 1)+','+this.pagesize+'")" data-page="' + (this.current - 1) + '" class="ui-pager">' + (this.current - 1) + '</li>';
					html += '<li onclick="pageTurning("'+(this.current)+','+this.pagesize+'")" data-page="' + this.current + '" class="ui-pager">' + this.current + '</li>';
					html += '<li onclick="pageTurning("'+(this.current + 1)+','+this.pagesize+'")" data-page="' + (this.current + 1) + '" class="ui-pager">' + (this.current + 1) + '</li>';
					html += '<li onclick="pageTurning("'+(this.current + 2)+','+this.pagesize+'")" data-page="' + (this.current + 2) + '" class="ui-pager">' + (this.current + 2) + '</li>';
					html += '<li class="ui-paging-ellipse" class="ui-pager">' + this.settings.ellipseTpl + '</li>';
				} else {
					html += '<li class="ui-paging-ellipse" >' + this.settings.ellipseTpl + '</li>';
					for (var i = this.pagecount - 2; i < this.pagecount; i++) {
						html += '<li onclick="pageTurning("'+i+','+this.pagesize+'")" data-page="' + i + '" class="ui-pager">' + i + '</li>';
					}
				}
				html += '<li onclick="pageTurning('+this.pagecount+','+this.pagesize+')"  data-page="' + this.pagecount + '" class="ui-pager">' + this.pagecount + '</li>';
			} else {
				for (var i = 1; i <= this.pagecount; i++) {
					html += '<li onclick="pageTurning('+i+','+this.pagesize+')" data-page="' + i + '" class="ui-pager">' + i + '</li>';
				}
			}
			html += '<li onclick="pageTurning('+(this.current + 1)+','+this.pagesize+')" class="js-page-next js-page-action ui-pager">' + this.settings.nextTpl + '</li>';
			html += '<li onclick="pageTurning('+(this.pagecount)+','+this.pagesize+')" class="js-page-last js-page-action ui-pager">' + this.settings.lastTpl + '</li>';
			html += '</ul>';
		...
		bindToolbar: function() {
			var _this = this;
			var html = $('<li class="ui-paging-toolbar"><select id="selectPage" onchange="pageTurning('+this.current+',this.value)" class="ui-select-pagesize"></select><input type="text" id="jump" class="ui-paging-count"/><a href="javascript:void(0)">跳转</a></li>');
			...
			$('input', html).click(function() {
				...
				if (e.keyCode == 13) {
					...
					pageTurning(current,this.pagesize);
					_this.go(current);
				}
			});
			$('a', html).click(function() {
				var current = parseInt($(this).prev().val()) || 1;
				pageTurning(current,$("#selectPage option:selected").val());
				_this.go(current);
			});
			sel.change(function() {
				_this.changePagesize(this.pagesize);
